...
def feature_trans_2(feature):
    ...
    """
    reassigned_feature = torch.cat((
        feature[:, [0, 3, 5, 7, 9, 12, 14, 16, 18, 21, 23, 25, 28]],

        feature[:, [1, 4, 6, 8, 10, 13, 15, 17, 19, 22, 24, 26, 29]]
    ), dim=1)
...
def location_trans_2(location):
    ...
    """
    reassigned_location = torch.cat((
        location[[0, 3, 5, 7, 9, 12, 14, 16, 18, 21, 23, 25, 28]],

        location[[1, 4, 6, 8, 10, 13, 15, 17, 19, 22, 24, 26, 29]]
    ), dim=0)
...
def feature_trans_7(feature):
    ...
    reassigned_feature = torch.cat((
        ...
        feature[:, [12, 14, 18, 21, 23]],

        feature[:, 2:6], feature[:, 7:9], feature[:, 11:14],
        feature[:, 16:18], feature[:, 20:23],

        feature[:, [4, 6, 10, 13, 15]],

        feature[:, [13, 15, 19, 22, 24]],
...
def location_trans_7(location):
    ...
    reassigned_location = torch.cat((
        ...
        location[[3, 5, 9, 12, 14]],

        location[[12, 14, 18, 21, 23]],

        location[2:6], location[7:9], location[11:14],
        location[16:18], location[20:23],

        location[[4, 6, 10, 13, 15]],

        location[[13, 15, 19, 22, 24]],

        location[25:30]), dim=0)
